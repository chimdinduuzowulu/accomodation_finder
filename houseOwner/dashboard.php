...
$uploadCount=$totalTenant="";
include "../server/connect.php";
$StudentTotal=$stdsSubmitted="";
$fetch = $conn->query("SELECT  uploadCount  FROM HouseOwner WHERE HouseOwner.HouseOwner_Id='$HouseOwner_Id'");
            if ($fetch){
                while($row=$fetch->fetch_assoc()){
                  $uploadCount=$row["uploadCount"]." ";
              }}
// get the students that have submitted their reports
$fetch2 = $conn->query("SELECT COUNT(tenantId) AS totalT
                    FROM TenantTb
                    WHERE HouseOwner_Id='$HouseOwner_Id'
                    ");
                    if ($fetch2){
                   while($row =$fetch2->fetch_assoc()){
                   $totalTenant=$row["totalT"]." ";}}
?>
          ...
          <div class="card">
            <div class="card-header p-3 pt-2">
              ...
              <div class="text-end pt-1">
                <p class="text-sm mb-0 text-capitalize">Total Houses</p>
                <h4 class="mb-0"><?php echo $uploadCount?></h4>
              </div>
            </div>
            <hr class="dark horizontal my-0">
            <div class="card-footer p-3">
              <p class="mb-0"><span class="text-success text-sm font-weight-bolder"><?php echo $uploadCount?></span>Published Houses</p>
            </div>
          </div>
        </div>
        <div class="col-xl-3 col-sm-6 mb-xl-0 mb-4">
          <div class="card">
            <div class="card-header p-3 pt-2">
              ...
              <div class="text-end pt-1">
                ...
                <h4 class="mb-0"><?php echo $totalTenant?></h4>
              </div>
            </div>
            <hr class="dark horizontal my-0">
            <div class="card-footer p-3">
              <p class="mb-0"><span class="text-success text-sm font-weight-bolder"><?php echo $totalTenant?> </span>Total Tenants</p>
            </div>
          </div>
        </div>
